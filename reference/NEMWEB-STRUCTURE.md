...
**Legit nested sub-dataset structure.** 8 CURRENT dirs and 5 ARCHIVE dirs contain named child datasets the consumer needs to enumerate separately:
...
| 12 other CURRENT dirs (see below) | 1 each | Pure single-file `_LEGACY.zip` placeholders. |
| **Total** | **652** | |

The 12 single-file DUPLICATE placeholders: `DISPATCH_NEGATIVE_RESIDUE`, `Dispatch_IRSR`, `Dispatch_Reports`, `Dispatch_SCADA`, `Dispatchprices_PRE_AP`, `P5_Reports`, `P5MINFCST`, `Predispatch_Reports`, `Predispatch_Sensitivities`, `SEVENDAYOUTLOOK_FULL`, `TradingIS_Reports`, `Yesterdays_Bids_Reports`. Each holds one `PUBLIC_*_LEGACY.zip` file. The parent streams themselves publish under a `..._LEGACY.zip` filename in CURRENT (see `reference/URL-CONVENTIONS.md` — the `PUBLIC_DISPATCH_{ts}_{dt}_LEGACY.zip` pattern for `Dispatch_Reports/CURRENT/`, etc.), so the single-file DUPLICATE is a 1-copy self-reference of that legacy-format convention.
...
1. **Single-file `_LEGACY.zip` placeholders** (12 cases) — exactly one file, always contains `_LEGACY` in the filename, matches the parent stream's documented `LEGACY` filename convention. Look like self-references / placeholders from a format-migration event. Not live data.
2. **Multi-file straggler subtrees** (5 cases with 2-9 files each: `MCCDispatch`, `Next_Day_Intermittent_DS`, `Next_Day_PreDispatch`, `PredispatchIS_Reports`, `Trading_Cumulative_Price`) — **no `_LEGACY` suffix**. Filenames match parent's non-legacy regex exactly. Timestamps are sparse across years, suggesting one-off orphans from specific events, not continuous streams. Structurally identical to parent content; unclear whether to treat as data or residue.
3. **`GBB/DUPLICATE/`** (617 files) — a **rolling timestamped archive** paired with the parent's live stream. Filenames are parent-dataset names with optional `_{yyyymmddHHMMSS}` suffix. Parent `GBB/` holds one current copy of each file; `GBB/DUPLICATE/` holds historical timestamped copies. Functionally this is an append-only archive masquerading as a DUPLICATE subtree, operationally indistinguishable from a live dataset.

Any catalog filter touching `/DUPLICATE/` must distinguish these three sub-cases — an unconditional skip regresses `GBB/DUPLICATE`'s 617 files and the 23 multi-file stragglers, total 640 files lost. The `_LEGACY` suffix presence is a reliable discriminator: present = placeholder (category 1), absent = real files (categories 2 and 3).
