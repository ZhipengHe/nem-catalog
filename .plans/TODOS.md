...
Tracked on GitHub: issue #12 (unclassified CURRENT paths), #13 (ARCHIVE retention class), #14 (pre-tag gate), #15 (list_urls), #16 (observed-range). Issue #17 was split into #21 + #22 on 2026-04-25 after Codex adversarial review separated a 5-line classifier fix from a schema-breaking emission-shape redesign.
...
|---|---|---|
| **P1a — SDK promise from v0.1.0** | #15 `list_urls()` | CHANGELOG promised this as v0.2 focus. Now the top unshipped item. |
| **P1b — schema add, dependency likely unmet** | #16 observed-range retention | Acceptance requires "≥6 weeks weekly workflow green"; weekly stabilized 2026-04-21. **Slip to v0.2.1 unless dependency is overridden.** Schema is already at 2.0.0; #16's delta extends an existing major. |
...
- **Minimum viable (honors v0.1.0 promise):** #15 (`list_urls()`). Adds the SDK feature v0.1.0 promised.
- **Stretch:** #15 + #13 — small-scope policy correctness fix on top of the SDK feature.
...
Candidate detail sections below are ordered to match this priority — P1a first, P3b last.
...
- **Depends on:** Weekly workflow reliably green for ≥6 weeks. Weekly stabilized on 2026-04-21 (PR #10); earliest ready date is ~2026-06-02. **Likely misses v0.2 unless dependency is overridden.** Schema is already at 2.0.0 from this PR; #16's delta extends an existing major without re-breaking consumers.
