...
def test_incompatible_error_message_has_docs_url():
    err = IncompatibleCatalogError(
        "catalog schema version 2.0.0 not supported by SDK 0.1 (supports 1.x)"
    )
...
def test_unresolvable_error_message_has_docs_url():
    err = UnresolvableDatasetError(
        "Reports:NEXT_DAY_OFFER_ENERGY)SPARSE is a directory-level anomaly"
    )
    assert "See:" in str(err)
